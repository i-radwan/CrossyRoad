...
void main() {
    float zPos = ((isSafeLane == 1.0f) ? position.z * 0.8: position.z * 2);
    if(isNormalLaneBeforeNormalLane == 1.0f){
        zPos = position.z;
    }
    gl_Position = projection * view * model * vec4(position.x * 50, ((isSafeLane == 1.0f) ? position.y+0.2f:position.y), zPos, 1.0f);
