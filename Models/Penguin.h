...
public:
    ...
    collisionStatus detectCollision(vector<lane> &lanesArray) {
        ...
        if (lanesArray[getCurrentLane()].hasCoin) {
            ...
            if ((penLeftPos < coinRightPos)
                && (penRightPos >= coinLeftPos)) {
                
                coinCollided = true;
                
            }
            
            if ((penRightPos >= coinLeftPos)
    ...
    void score(vector<lane> &lanesArray, long &score) {
        ...
        if (lanesArray[getCurrentLane()].type) {
            if (!(lanesArray[getCurrentLane()].isVisited)) {
                score++;
                lanesArray[getCurrentLane()].isVisited = true;
            }
